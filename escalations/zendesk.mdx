...
### Step 6: Configure Webhook (Required for Agent Replies)
...
Then create a **Trigger** to fire the webhook:

1. Go to **Admin Center** → **Objects and rules** → **Triggers**
2. Click **"Add trigger"**
3. Configure:
   - **Name**: "Support Unicorn Agent Reply"
   - **Conditions** (Meet ALL):
     - Ticket > Tags > Contains at least one of > `ai-escalation` (or your configured tag)
     - Comment > Is > Public
...
<Warning>
  Without this webhook, ticket creation will work but **agent replies will not reach the customer**. The webhook is what enables bi-directional communication.
</Warning>

### Step 7: Use a Separate API User (Important)
